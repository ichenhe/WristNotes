...
//						startActivity(mainint);
    ...
    public static void openNovel(SharedPreferences sp, String path, String name)
    {
		try
		{
			...
			List<String> novelpath = Arrays.asList(novellist.getString(name).split("$♂$"));
			for (int i = 0; i < novellist.getString(name).split("$♂$").length; i++)
			{
			...
			if (j == 0)//第一次打开
			{
				...
				sp.edit().commit();
				MainActivity.mode = 1;
				Toast.makeText(fileselectCtx, "已打开小说" + name.split(".")[0], Toast.LENGTH_SHORT);
			}
			else
			{
